...
let initialize l pip =
  ...
  sumBC.value := sum_orderedQ_XY l 1 2;
...
let p = !(res.content);;
  ...
  (*
let rec string_of_int_list_local l s =
  match l with
  | [] -> ""
  | [alone] -> (" " ^ (string_of_int alone) ^ ";")
  | head::tail -> string_of_int_list_local tail (" " ^ (string_of_int head) ^ ";") in
    let string_of_int_list l =
      match l with
      | [] -> "[]"
      | _ ->
         let s = string_of_int_list_local l "";
                 begin
                   set_nth_char s 0 '[';
                   set_nth_char s (string_length l)-1 ']';
                   s;
                 end;;
   *)
...
(*
(*Execution times*)
let a = calculate_ratio_A 2 2;;
let b = calculate_ratio_B 2 2;;
let c = calculate_ratio_C 2 2;;
...
(*
create_file_plot "johnson_gnuplot.txt" (range 1 10) a b c;; 
*)
